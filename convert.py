...
Use is simple:
1) create a file called convert.in, and place the block of C# code
   in it (This utility works best one function at a time).
2) run this script (./convert.py)
3) the results of the conversion will be written to convert.out.

NOTE: there is a mode that makes a fair attempt at converting .aspx
files to standard html.
    ...
    with open(in_path, 'r') as f_in:
        ...
        for line in f_in:
            ...
            if (len(sys.argv) > 1):
                ...
                #   we wanna count the whitespace at the beginning of
                #   the line might use it later for indent validation,
                #   etc.
                p = re.compile(r"^\s+")
                m = p.match(line)
                if m:
                ...
                sINDENT = " " * INDENT
                # This string global contains the current indent level
                #   + 4
                sINDENTp4 = " " * (INDENT+4)
                # + 8
                sINDENTp8 = " " * (INDENT+8)
                ...
                # doing "string" and "int" again down below with a
                #   regex, because they could be a line starter, or part
                #   of a bigger word
                line = line.replace(" int ", " ").replace(
                    " string ",
                    " "
                ).replace(" bool ", " ").replace(" void ", " ")
                line = line.replace("(int ", "(").replace(
                    "(string ",
                    "("
                ).replace("(bool ", "(")
                ...
                line = line.replace("catch (Exception ex)",
                                    "except Exception:")
                # I often threw "new exceptions" - python doesn't need
                #  the extra stuff
                ...
                line = line.replace("HttpContext.Current.Server.MapPath"
                                    "(", "")
                # ^ This will leave a trailing paren!
                line = line.replace(").Value", ", \"\")
                # ^ WAS A .Value - confirm")
                #   #should work most of the time for Cato code
                line = line.replace(".Length", ".__LENGTH")

                # These commonly appear on a line alone, and we aren't
                #   using them any more.
                if line.strip() == "dataAccess dc = new dataAccess()":
                    continue
                if line.strip() == "acUI.acUI ui = new acUI.acUI()":
                    continue
                if line.strip() == "sErr = \"\"": continue
                if line.lower().strip() == "ssql = \"\"": continue
                # ^ there's mixed case usage of "sSql"
                ...
                if "FunctionTemplates.HTMLTemplates ft" in line:
                    continue


                # a whole bunch of common phrases from Cato C# code
                line = line.replace("Globals.acObjectTypes",
                                    "uiGlobals.CatoObjectTypes")
                line = line.replace("ui.", "uiCommon.")
                line = line.replace("ft.", "ST.")
                # ^ "FunctionTemplates ft is now import stepTemplates as sST"
                line = line.replace("dc.IsTrue", "uiCommon.IsTrue")
                line = line.replace("../images", "static/images")
                line = line.replace("dc.EnCrypt",
                                    "catocommon.cato_encrypt")
                ...
                if "!dc.sqlGetSingleString" in line:
                    line = (sINDENT + "00000 = uiGlobals.request.db"
                            ".select_col_noexcep(sSQL)\n" + sINDENT
                            + "if uiGlobals.request.db.error:\n")

                if "!dc.sqlGetDataTable" in line:
                    line = (sINDENT + "00000 = uiGlobals.request.db"
                            ".select_all_dict(sSQL)\n" + sINDENT
                            + "if uiGlobals.request.db.error:\n")

                line = line.replace(
                    "+ sErr",
                    "+ uiGlobals.request.db.error"
                )
                # ^ + sErr is *usually used for displaying a db error.
                #   Just switch it.
                line = line.replace(
                    "(sErr)",
                    "(uiGlobals.request.db.error)"
                )  # sometimes it's in a log message as the only arg
                line = line.replace(
                    "if (!oTrans.ExecUpdate(ref sErr))",
                    ("if not uiGlobals.request.db"
                     ".tran_exec_noexcep(sSQL):")
                )
                line = line.replace("oTrans.Command.CommandText",
                                    "sSQL")
                # ^ transactions are different, regular sSQL variable
                line = line.replace(
                    "oTrans.Commit()",
                ...
                line = line.replace(
                    "oTrans.RollBack()",
                    "uiGlobals.request.db.tran_rollback()"
                )
                ...
                tmp = "if (!dc.sqlGetDataRow(ref dr, sSQL, ref sErr))"
                if line.strip() == tmp:
                    line = (sINDENT + "dr = uiGlobals.request.db"
                            + ".select_row_dict(sSQL)\n" + sINDENT
                            + "if uiGlobals.request.db.error:\n"
                            + sINDENTp4 + "raise Exception(uiGlobals"
                            + ".request.db.error)\n")
                tmp = "if (!dc.sqlGetDataTable(ref dt, sSQL, ref sErr))"
                if line.strip() == tmp:
                    line = (sINDENT + "dt = uiGlobals.request.db"
                            + ".select_all_dict(sSQL)\n" + sINDENT
                            + "if uiGlobals.request.db.error:\n"
                            + sINDENTp4 + "raise Exception("
                            + "uiGlobals.request.db.error)\n")
                ...
                # xml/Linq stuff
                # The following lines were useful, but NOT foolproof, in
                #   converting some Linq XDocument/XElement stuff
                ...
                line = line.replace(".XPathSelectElements", ".findall").replace(
                    ".XPathSelectElement",
                    ".find"
                )
                ...
                '''
                ##### CUSTOM #####
                # !!! this has to be done after the database stuff,
                #   because they all use a "ref sErr" and we're matching
                #   on that!
                # passing arguments by "ref" doesn't work in python,
                #   mark that code obviously
                # because it need attention
                line = line.replace("ref ", "0000BYREF_ARG0000")

                # the new way of doing exceptions - not raising them,
                #   appending them to an output object
                line = line.replace(
                    "raise ex",
                    "uiGlobals.request.Messages.append(traceback.format_exc())"
                )
                line = line.replace(
                    "raise Exception(", "uiGlobals.request.Messages.append("
                )


                # if this is a function declaration and it's a "wm" web
                #   method,
                ...
                # foreach statements (also marking them because type
                #   declarations may need fixing)
                p = re.compile(r".*foreach \(.*\)")
                m = p.match(line)
                if m:
                ...
                if m:
                    ...
                    p = re.compile(r"\(.*\?")
                    m = p.search(line)
                    if_part = m.group().replace("(", "").replace(
                        "?",
                        ""
                    ).replace(")", "").strip()
                    ...
                    p = re.compile(r"\?.*:")
                    m = p.search(line)
                    then_part = m.group().replace(":", "").replace(
                        "?",
                        ""
                    ).strip()
                    ...
                    p = re.compile(r":.*\)")
                    m = p.search(line)
                    else_part = m.group().replace(":", "").replace(
                        "?",
                        ""
                    ).replace(")", "").strip()

                    # now reconstitute it (don't forget the rest of the line
                    post_munge = "(%s if %s else %s)" % (then_part, if_part,
                                                         else_part)
...
def main():
    convert_cs_to_python_file('convert.in', 'convert.out')
    return 0
